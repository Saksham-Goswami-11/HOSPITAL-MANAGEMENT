...
Deno.serve(async (req: Request) => {
    ...
    try {
        ...
        const { action, hospitalId, requestId, approve } = await req.json();
        ...
        if (['approve_trial_extension', 'pause', 'resume', 'delete_hospital'].includes(action) && !isSuperAdmin) {
        ...
        switch (action) {
            case 'approve_trial_extension': {
                ...
                if (approve) {
                    ...
                    // Calculate new trial end (+8 days)
                    ...
                    console.log('Trial extension approved for:', hospitalId);
                    return new Response(JSON.stringify({ success: true, message: 'Trial extended by 8 days' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
                } else {
                    await supabaseAdmin
                        .from('trial_extension_requests')
