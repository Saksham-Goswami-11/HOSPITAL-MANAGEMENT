...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white/90 backdrop-blur-md shadow-sm py-3 border-b border-slate-100' : 'bg-white py-4 border-b border-transparent'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            <Link to="/contact" className="bg-primary hover:opacity-90 px-6 py-2.5 rounded-xl text-sm font-semibold shadow-md shadow-blue-500/20 transition-all text-white">
              Get Started
            </Link>
          </div>
      ...
      {isOpen && (
        <div className="md:hidden fixed inset-x-0 top-[73px] bg-white border-b border-slate-100 p-6 space-y-6 shadow-2xl z-50 animate-in fade-in slide-in-from-top-4 flex flex-col h-[calc(100vh-73px)] overflow-y-auto">
          ...
          <div className="pt-4 mt-auto pb-8">
            <Link to="/contact" className="block w-full bg-primary hover:bg-blue-700 text-white px-6 py-4 rounded-xl font-bold text-center shadow-lg transition-colors">
              Get Started
            </Link>
          </div>
        </div>
